...
<Image
  imgName="cmdquery.png"
  alt="Slug in Command Line"
...
Eh? _Query de página_??? Sí, en Gatsby hay dos formas de consultar data:

- **Normal (o Page) Queries** (Query de página)
- **Static Queries** (Query estática)

Una de las principales diferencias entre las dos es que la _Query de página_
puede usarse solamente en los componentes de **página**, y era la única forma de
consultar data en Gatsby v1. Luego desarrollaron la _Query estática_, que
apareció en Gatsby v2 y fue fundamental para eliminar lo que se llama _prop
drilling_, que es pasar data (o state) mediante props de padre a hijos en
múltiples niveles, suponiendo que a veces esa data se pasaba unas 3 ó 4 (o más)
veces, lo cual volvía el código inmantenible. Las _Static Queries_ pueden
utilizarse tanto en componentes **comunes** como en componentes de **página**.

Y la última gran diferencia es que la _Query de página_ **acepta variables**,
las cuales se reciben mediante el _contexto de página_ o **pageContext**, el cual 
...
[@AgustinDMulet](https://twitter.com/AgustinDMulet), nos vemos gente!!!
